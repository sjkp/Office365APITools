...
namespace Office365APIToolsSample
{
    static class  MyFilesApiSample
    {
        ...
        private static SharePointClient client = null;

        public static async Task<IEnumerable<IFileSystemItem>> GetMyFiles()
        {
            //var client = Office365Helper.OneDriveClient;

            //string filesUrl = String.Format(CultureInfo.InvariantCulture,
            //    "/web/lists/GetByTitle('Documents')/Files");

                       
            //var files = await client.Get<IEnumerable<FileSystemItem>>(filesUrl);
            if (client == null)
            {
                client = await EnsureClientCreated();
    ...
    public class OneDriveClient : Office365Client
    {
        ...
        private static AuthenticationInfo _authenticationInfo = null;
    ...
    public abstract class Office365Client
    {
        ...
        public async Task<T> Get<T>(string requestUrl) 
        {
            ...
            var res = await Execute(authInfo.ServiceUri + requestUrl, HttpMethod.Get);
